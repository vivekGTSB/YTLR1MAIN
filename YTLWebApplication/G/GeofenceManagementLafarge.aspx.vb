...
Partial Class GeofenceManagementLafarge
                            ...
                            If role <> "Admin" Then
                                If GetUserAttribute("LA") = "Y" Then
                                    ...
                                    If r(3).ToString() = "Polygon" Then
                                        If status Then
                                            r(2) = String.Format("<a href='UpdateGeofenceLafarge.aspx?geofenceid={0}'>{1}</a>", 
                                                               Server.UrlEncode(geofenceId), geofenceName)
                                        Else
                                            r(2) = String.Format("<a href='UpdateGeofenceLafarge.aspx?geofenceid={0}'><span style=""text-decoration: line-through; color: #FF0000"">{1}</span></a>", 
                                                               Server.UrlEncode(geofenceId), geofenceName)
                                        End If
                                    Else
                                        If status Then
    ...
    Private Sub BuildGridHtml(ByVal geofencetable As DataTable)
        Try
            sb1.Length = 0
            sb1.Append("<thead><tr><th align=""center"" style=""width:20px;""><input type='checkbox' onclick=""javascript:checkall(this);""/></th><th style=""width:35px;"">S No</th><th>Geofence Name</th><th style=""width:120px;"">Centroid</th><th style=""width:130px;"">Modify DateTime</th></tr></thead>")
                ...
                Try
                    ...
                    For j As Integer = 0 To 4 ' Only include necessary columns
                        ...
                        Select Case j
                            ...
                            Case 2
                                sb1.Append(geofencetable.DefaultView.Item(i)(2).ToString()) ' Already encoded
                            Case 3
                                sb1.Append(geofencetable.DefaultView.Item(i)(7).ToString()) ' Centroid - already encoded
                            Case 4
                                sb1.Append(HttpUtility.HtmlEncode(geofencetable.DefaultView.Item(i)(5).ToString()))
                        End Select
                        sb1.Append("</td>")
                    Next
                    sb1.Append("</tr>")
            ...
            sb1.Append("<tfoot><tr><th><input type='checkbox' onclick=""javascript:checkall(this);""/></th><th>S No</th><th>Geofence Name</th><th>Centroid</th><th>Modify DateTime</th></tr></tfoot>")
            sb1.Append("</tbody>")
    ...
    Private Function CalculateCentroid(ByVal data As String) As String
        Try
            ...
            Return shp.Centroid.Y.ToString("0.0000") & "," & shp.Centroid.X.ToString("0.0000")
        Catch
