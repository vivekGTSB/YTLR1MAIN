...
Partial Class SuperUserManagement
        ...
        Try
            ...
            If Page.IsPostBack = False Then
                ...
                If Convert.ToString(Request.Cookies("userinfo")("username")).ToUpper() = "SVWONG" Then
                    da = New SqlDataAdapter("select userid,username from userTBL where role='SuperUser' and companyname='LAFARGE' order by username", conn)
                Else
                    da = New SqlDataAdapter("select userid,username from userTBL where role='SuperUser' order by username", conn)
                End If
    ...
    Protected Sub Fill()
        Try
            If ddluser.SelectedValue <> "--Select Super User--" Then
                ...
                Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("sqlserverconnection"))
                Dim cmd As SqlCommand = New SqlCommand("select username,userslist from userTBL where userid='" & ddluser.SelectedValue & "' and username<>'bintangown'", conn)
                ...
                cmd = New SqlCommand("select userid,username from userTBL where role='User' and userid not in(" & usersstring & ") and username<>'bintangown' order by username", conn)
                dr = cmd.ExecuteReader()
