...
Partial Class VehicleIdlingSummaryReport
    ...
    Protected Overrides Sub OnInit(ByVal e As System.EventArgs)
        Try
            ...
            For i As Int16 = 0 To 6
                If Month(DateAdd(DateInterval.Month, -i, Date.Now)) >= 2 And Year(DateAdd(DateInterval.Month, -i, Date.Now)) > 2013 Then
                    yearmonth = MonthName(Month(DateAdd(DateInterval.Month, -i, Date.Now))).ToString().Substring(0, 3) & " " & Year(DateAdd(DateInterval.Month, -i, Date.Now)).ToString()
                    ddlMonth.Items.Add(New ListItem(yearmonth, Year(DateAdd(DateInterval.Month, -i, Date.Now)).ToString() & "-" & Month(DateAdd(DateInterval.Month, -i, Date.Now)).ToString()))
                End If
            Next
    ...
    '        cmd = New SqlCommand("select plateno,SUM(duration) as idlingtime from vehicle_movement where  type=1 and MONTH (fromtime)="&  &" group by plateno  order by plateno ", conn)
    '        conn.Open()
    ...
    '        While dr.Read()
    '            span = TimeSpan.FromSeconds(Convert.ToDouble(dr("duration")))
    '            If span.TotalMinutes >= minOption Then
    '                r = t.NewRow
    '                r(0) = i
    '                r(1) = dr("plateno")
    '                r(2) = Convert.ToDateTime(dr("fromtime")).ToString("yyyy-MM-dd HH:mm:ss")
    '                r(3) = Convert.ToDateTime(dr("totime")).ToString("yyyy-MM-dd HH:mm:ss")
    '                r(4) = span
    ...
    '                End If
    '                r(7) = "<a href='http://maps.google.com/maps?f=q&hl=en&q=" & dr("frmlat") & " + " & dr("frmlon") & "&om=1&t=k' target='_blank'><img style='border:solid 0 red;' src='images/googlemaps1.gif' title='View map in Google Maps'/></a>   <a href='GoogleEarthMaps.aspx?x=" & dr("frmlon") & "&y=" & dr("frmlat") & "'><img style='border:solid 0 red;' src='images/googleearth1.gif' title='View map in GoogleEarth'/></a>"
    '                r(9) = dr("frmlat")
    '                r(10) = dr("frmlon")
    '                t.Rows.Add(r)
    '                i = i + 1
    ...
    Protected Sub displayidlingSummary()
        Try
            ...
            Dim r As DataRow
            Dim yearmonth As String()
            yearmonth = ddlMonth.SelectedValue.Split("-")
            'GridView1.Columns.Item(1).Visible = False
            If ddlpleate.SelectedValue <> "ALL" Then
                cmd = New SqlCommand("select plateno,SUM(duration) as idlingtime from vehicle_movement where  type=1 and MONTH (fromtime)=" & yearmonth(1) & " and year (fromtime)=" & yearmonth(0) & " and plateno='" & ddlpleate.SelectedValue & "' group by plateno  order by plateno ", conn)
            Else
                cmd = New SqlCommand("select plateno,SUM(duration) as idlingtime from vehicle_movement where  type=1 and MONTH (fromtime)=" & yearmonth(1) & " and year (fromtime)=" & yearmonth(0) & " and plateno in (select plateno from vehicleTBL where userid='" & ddlUsername.SelectedValue & "') group by plateno  order by plateno ", conn)
            End If
             conn.Open()
            Dim dr As SqlDataReader = cmd.ExecuteReader()
            Dim span As TimeSpan
            ...
            While (dr.Read())
                ...
                If span.Days > 0 Then
                    displaytext = span.Days.ToString.PadLeft(2, "0"c) & " Days " & span.Hours.ToString.PadLeft(2, "0"c) & ":" & _
                        span.Minutes.ToString.PadLeft(2, "0"c) & ":" & span.Seconds.ToString.PadLeft(2, "0"c)
                Else
                    displaytext = "00 Days " & span.Hours.ToString.PadLeft(2, "0"c) & ":" & _
                       span.Minutes.ToString.PadLeft(2, "0"c) & ":" & span.Seconds.ToString.PadLeft(2, "0"c)
                End If
                r(2) = displaytext
            ...
            If totaltime.Days > 0 Then
                displaytext = totaltime.Days.ToString.PadLeft(2, "0"c) & " Days " & totaltime.Hours.ToString.PadLeft(2, "0"c) & ":" & _
                    totaltime.Minutes.ToString.PadLeft(2, "0"c) & ":" & totaltime.Seconds.ToString.PadLeft(2, "0"c)
            Else
                displaytext = "00 Days " & totaltime.Hours.ToString.PadLeft(2, "0"c) & ":" & _
                   totaltime.Minutes.ToString.PadLeft(2, "0"c) & ":" & totaltime.Seconds.ToString.PadLeft(2, "0"c)
            End If
            r(2) = displaytext
            t.Rows.Add(r)
