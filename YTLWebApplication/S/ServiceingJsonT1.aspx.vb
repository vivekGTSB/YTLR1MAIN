...
Partial Class ServiceingJsonT1
    ...
    Private Sub GetTicketInformation(ByVal UID As String, ByVal role As String)
        ...
        Try
            ...
            If role = "Admin" Or role = "AdminViewer1" Or role = "AdminViewer2" Then
                cmd = New SqlCommand("select vs.userid,ut.username,vs.serviceid,vs.plateno,convert(varchar(19),vs.timestamp,120) as timestamp,vs.serexpdtime,dbo.fn_getvehicletype(vs.plateno) as type ,vs.servicetype,vs.priority,vs.status,isnull(vsh.assignedto,'Not at Assigned') assignedto,vsh.servicedate,vs.assignedby,vs.remarks,dbo.fn_getUnreadcomments(vs.serviceid,'2057') as unread from vehicle_services vs left outer join vehicle_services_history vsh on vs.serviceid=vsh.serviceid inner join userTBL ut on vs.userid=ut.userid where vs.timestamp between DATEADD (MONTH,-6,GETDATE()) and GETDATE() order by timestamp desc", conn)

            Else
                If userid = "1967" Then
                    cmd = New SqlCommand("select vs.userid,ut.username,vs.serviceid,vs.plateno,convert(varchar(19),vs.timestamp,120) as timestamp,vs.serexpdtime,dbo.fn_getvehicletype(vs.plateno) as type ,vs.servicetype,vs.priority,vs.status,isnull(vsh.assignedto,'Not at Assigned') assignedto,vsh.servicedate,vs.assignedby,vs.remarks,dbo.fn_getUnreadcomments(vs.serviceid,'1967') as unread from vehicle_services vs left outer join vehicle_services_history vsh on vs.serviceid=vsh.serviceid inner join userTBL ut on vs.userid=ut.userid where vs.timestamp between DATEADD (MONTH,-6,GETDATE()) and GETDATE() order by timestamp desc", conn)
                Else
                    cmd = New SqlCommand("select vs.userid,dbo.[fn_getusername](vs.userid) as username ,vs.serviceid,vs.plateno,convert(varchar(19),vs.timestamp,120) as timestamp,vs.serexpdtime,dbo.fn_getvehicletype(vs.plateno) as type ,vs.servicetype,vs.priority,vs.status,isnull(vsh.assignedto,'Not at Assigned') assignedto,vsh.servicedate,vs.assignedby ,vs.remarks,dbo.fn_getUnreadcomments(vs.serviceid,vs.userid) as unread from vehicle_services vs left outer join vehicle_services_history vsh on vs.serviceid=vsh.serviceid where userid='" & userid & "' order by timestamp desc", conn)
                End If
            End If
