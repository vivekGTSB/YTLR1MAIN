...
Partial Class VehiclesJson
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Dim compass() As String = {"N", "22", "NE", "67.5", "E", "112.5", "SE", "157.5", "S", "202.5", "SW", "247.5", "W", "292.5", "NW", "337.5", "N"}
            ...
            If plateno = "" Then
                If role = "User" Then
                    da = New SqlDataAdapter("select ut.username,vtt.panic,vtt.powercut,vtt.overspeed,vt.pto,vtt.alarm,vt.plateno,vt.groupname,vt.type,convert(varchar(19),vtt.timestamp,120) as datetime,gps_odometer,speed,gps_av,lat,lon,ignition_sensor,vt.smallimage,vt.bigimage,vtt.bearing from  vehicle_tracked vtt,vehicleTBL vt,userTBL ut where vtt.plateno=vt.plateno and vt.userid=ut.userid and vt.userid='" & userid & "'", conn)
                ElseIf role = "SuperUser" Or role = "Operator" Then
                    da = New SqlDataAdapter("select ut.username,vtt.panic,vtt.powercut,vtt.overspeed,vt.pto,vtt.alarm,vt.plateno,vt.groupname,vt.type,convert(varchar(19),vtt.timestamp,120) as datetime,gps_odometer,speed,gps_av,lat,lon,ignition_sensor,vt.smallimage,vt.bigimage,vtt.bearing from  vehicle_tracked vtt,vehicleTBL vt,userTBL ut where vtt.plateno=vt.plateno and vt.userid=ut.userid and vt.userid in(" & userslist & ")", conn)
                Else
                    da = New SqlDataAdapter("select ut.username,vtt.panic,vtt.powercut,vtt.overspeed,vt.pto,vtt.alarm,vt.plateno,vt.groupname,vt.type,convert(varchar(19),vtt.timestamp,120) as datetime,gps_odometer,speed,gps_av,lat,lon,ignition_sensor,vt.smallimage,vt.bigimage,vtt.bearing from  vehicle_tracked vtt,vehicleTBL vt,userTBL ut where vtt.plateno=vt.plateno and vt.userid=ut.userid", conn)
                End If
            Else
                If searchin = "vht" Then
                    da = New SqlDataAdapter("select top 1 ut.username,vht.panic,vht.powercut,vht.overspeed,vht.alarm,vt.pto,vt.plateno,vt.groupname,vt.type,convert(varchar(19),vht.timestamp,120) as datetime,gps_odometer,speed,gps_av,lat,lon,ignition_sensor,smallimage,bigimage,vht.bearing from  vehicle_history vht,vehicleTBL vt,userTBL ut where vt.userid=ut.userid and vht.plateno like '" & plateno & "%' and  gps_av = 'A' and vht.timestamp between '" & bdt & "' and '" & edt & "' and vt.plateno like'" & plateno & "%' order by vht.timestamp desc", conn)
                Else
                    da = New SqlDataAdapter("select ut.username,vt.plateno,vht.alarm,vt.pto,vt.groupname,vt.type,convert(varchar(19),vht.timestamp,120) as datetime,gps_odometer,speed,gps_av,lat,lon,ignition_sensor,smallimage,bigimage,bearing from  vehicle_tracked vht,vehicleTBL vt,userTBL ut where vht.plateno like'" & plateno & "%' and vt.plateno like '" & plateno & "%' and vt.userid=ut.userid", conn)
                End If
            End If
            ...
            For i As Int32 = 0 To ds.Tables(0).Rows.Count - 1
                Try
                    ...
                    If ds.Tables(0).Rows(i)("pto") Then
                        'If True Then
                        If ds.Tables(0).Rows(i)("alarm") Then
                            pto = 1
                        Else
                            pto = 0
                        End If
                    ...
                    Else
                        ...
                        a.Add(0)
                    End If
                    compassindex = ((ds.Tables(0).Rows(i)("bearing") - 22.5 + 22.5) / 22.5)
