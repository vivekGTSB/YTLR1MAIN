...
Partial Class AddInstantAlertSettings
    ...
    Protected Overrides Sub OnInit(ByVal e As System.EventArgs)
        Try
            If Page.IsPostBack = False Then
                ...
                cmd = New SqlCommand("select plateno from vehicleTBL where  userid='" & ddluser.SelectedValue & "' and plateno not in (select plateno from instant_alert_settings where userid='" & ddluser.SelectedValue & "')  order by plateno", conn)
    ...
    Protected Sub ImageButton2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton2.Click
        Try
            ...
            If Request.Form("chk") <> Nothing Then
                ...
                Attach = Convert.ToByte(splitstrbin(8), 2)
                Detach = Convert.ToByte(splitstrbin(9), 2)
                Towing = Convert.ToByte(splitstrbin(10), 2)
            End If
            conn.Open()
            Try
                For i As Int32 = 0 To CheckBoxList1.Items.Count - 1
                    Try
                        ...
                        If CheckBoxList1.Items.Item(i).Selected = True Then
                            ...
                            cmd = New SqlCommand("insert into instant_alert_settings(userid,plateno,mobileno,emailid,alarm,geofencein,geofenceout,idling,immobilizer,overspeed,panic,powercut,idlingtime,attach,detach,towing) values('" & userid & "','" & CheckBoxList1.Items.Item(i).Text & "','" & strmobile & "','" & stremail & "'," & ptoon & "," & geofencein & "," & geofenceout & "," & Idling & "," & Immobilizer & "," & Overspeed & "," & Panic & "," & Powercut & ",'" & txtIdlingTime.Value & "'," & Attach & "," & Detach & "," & Towing & ")", conn)
                            cmd.ExecuteNonQuery()
                        End If
                ...
                Response.Redirect("InstantAlertSettingsManagement.aspx")
    ...
    Protected Sub ddluser_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddluser.SelectedIndexChanged
        Try
            Dim conn As SqlConnection = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("sqlserverconnection"))
            Dim cmd As SqlCommand = New SqlCommand("select plateno from vehicleTBL where  userid='" & ddluser.SelectedValue & "' and plateno not in (select plateno from instant_alert_settings where userid='" & ddluser.SelectedValue & "')  order by plateno", conn)
            Try
                conn.Open()
                Dim dr As SqlDataReader = cmd.ExecuteReader()
