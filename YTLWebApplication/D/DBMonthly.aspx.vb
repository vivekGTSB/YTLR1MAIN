...
#End Region
    Protected Overrides Sub OnInit(ByVal e As System.EventArgs)
        Try
            ...
            'Dim strSql As String = "select userid, username,dbip from userTBL where role=@role order by username"
            ...
            ElseIf role = "Operator" Or role = "SuperUser" Then
                ' userlist must already be: "'u001','u002','u003'" (quoted and comma-separated)
                strSql = "SELECT userid, username, dbip FROM userTBL WHERE userid IN (" & userlist & ") ORDER BY username"
                sqlCmd.CommandText = strSql
            Else
                strSql = "SELECT userid, username, dbip FROM userTBL WHERE role = @role ORDER BY username"
                sqlCmd.CommandText = strSql
