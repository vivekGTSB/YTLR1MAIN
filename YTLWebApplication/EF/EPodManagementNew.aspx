<%@ Page Language="VB" AutoEventWireup="false" Inherits="YTLWebApplication.EPodManagementNew" Codebehind="EPodManagementNew.aspx.vb" %>
    ...
    <script type="text/javascript">
          ...
          $("#txtEndDate").datepicker({ dateFormat: 'yy/mm/dd', minDate: new Date(2013, 10, 01), maxDate: +0, changeMonth: true, changeYear: true, numberOfMonths: 2
            ...
            $("#MyDiv").dialog({
                ...
                buttons: {
                    Accept: function () {
                        if (confirm("Do you really want to Accept this Job ?")) {  
                            if (this.firstElementChild.contentDocument.getElementById("IsDiversion_0").checked) {
                                document.getElementById("hdnDiversion").value = "1";
                            }
                            else {
                                document.getElementById("hdnDiversion").value = "0";
                            }
                            UpdateData(2);
                            $(this).dialog("close");
                        }                        
                    ...
                    Reject: function () {
                        if (confirm("Do you really want to Reject this Job ?")) {
                            if (this.firstElementChild.contentDocument.getElementById("IsDiversion_0").checked) {
                                document.getElementById("hdnDiversion").value = "1";
                            }
                            else {
                ...
                $("#divRejectRemarks").dialog({
                    ...
                    buttons: {
                        Submit: function () {
                            ...
                            var lastXHR = $.get("UpdateJobStatusNew.aspx?d=" + document.getElementById("hdnDiversion").value +"&i=3" + "&p=" + patchno + "&rem=" + document.getElementById("txtRejectRemarks").value + "&rdb=" + $("#rdbRadio").find("input:checked").val(), function (data) {
                                if (data == "Yes") {
        ...
        function openMe(patchno) {
            ...
            document.getElementById("driverData").src= "ViewUploadedDataNew.aspx?p=" + patchno + "&r=" + Math.random();
            document.getElementById("driverData").style.visibility = "visible";
            $("#MyDiv").dialog("open");
        }
        function UpdateData(status_code) {
            //Code for Reject{3}, take input remarks
            

            if (status_code == 3) {
                // No Update here
                $("#divRejectRemarks").dialog("open");
            }
            else {
                var patchno = document.getElementById("hdnP").value;
                var lastXHR = $.get("UpdateJobStatusNew.aspx?d=" + document.getElementById("hdnDiversion").value  +"&i=" + status_code + "&p=" + patchno, function (data) {
                    if (data == "Yes") {
                        alert("Successfully updated the status.");
                        return false;
                    }
...
<body id="index" style="font-size: 11px; font-weight: bold; font-family: Myriad Pro,Lucida Grande,Helvetica,Arial,sans-serif">
    ...
    <div id="divRejectRemarks" title="Reject Remarks" style="padding-top: 1px; padding-right: 0px;
        padding-bottom: 0px; font-size: 10px; padding-left: 0px; font-weight: bold; font-family: Myriad Pro,Lucida Grande,Helvetica,Arial,sans-serif">
           <table>
               <tr>
                   <td><b style="color: #465AE8;"> Select One</b></td>
                   <td>
                       <asp:RadioButtonList runat="server" ID="rdbRadio">
                           <asp:ListItem Text="Image is not clear (Gambar tidak jelas)"  Value="Image is not clear (Gambar tidak jelas)"/>
                           <asp:ListItem Text="Image is incomplete (Gambar tidak lengkap)" Value="Image is incomplete (Gambar tidak lengkap)" />
                           <asp:ListItem Text="Wrong / No chop and sign (Salah / tiara tandatangan)" Value="Wrong / No chop and sign (Salah / tiara tandatangan)" />
                           <asp:ListItem Text="Others (Lain)" Value="Others (Lain)" Selected="True"  />
                       </asp:RadioButtonList>
                   </td>
               </tr>
               <tr>
                   <td> <b style="color: #465AE8;">Remarks</b></td>
                   <td>
                       <asp:TextBox runat="server" ID="txtRejectRemarks" Height="70" Width="400" TextMode="MultiLine" />
                   </td>
               </tr>
           </table>
    </div>
     <div class="demo">
         <input type="hidden" name="h1" value="" id="hdnP" />
          <input type="hidden" name="h2" value="" id="hdnDiversion" />
