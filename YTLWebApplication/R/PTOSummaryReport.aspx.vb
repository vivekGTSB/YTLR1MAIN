...
Partial Class PTOSummaryReport
    ...
    Protected Sub DisplayLogInformation()
        Try
            ...
            query = "select p.plateno,p.from_timestamp,p.to_timestamp,p.lat,p.lon,p.userid,u.username,v.groupname,v.pmid from pto_history p Join userTBL u on u.userid=p.userid Join vehicleTBL v On v.plateno=p.plateno and p.from_timestamp between '" & begindatetime & "' and '" & enddatetime & "' and v.pto='1'"
            If DropDownList1.SelectedValue <> "--ALL USERS--" Then
                If ddlplate.SelectedValue = "--ALL PLATES--" Then
                    query = "select p.plateno,p.from_timestamp,p.to_timestamp,p.lat,p.lon,p.userid,u.username,v.groupname,v.pmid from pto_history p Join userTBL u on u.userid=p.userid Join vehicleTBL v On v.plateno=p.plateno and p.userid= '" & DropDownList1.SelectedValue & "'  and p.from_timestamp between '" & begindatetime & "' and '" & enddatetime & "' and v.pto='1'"
                Else
                    query = "select p.plateno,p.from_timestamp,p.to_timestamp,p.lat,p.lon,p.userid,u.username,v.groupname,v.pmid from pto_history p Join userTBL u on u.userid=p.userid Join vehicleTBL v On v.plateno=p.plateno and p.plateno='" & ddlplate.SelectedValue & "' and p.from_timestamp between '" & begindatetime & "' and '" & enddatetime & "' and v.pto='1'"
                End If
            Else
                If ddlplate.SelectedValue = "--ALL PLATES--" Then
                    If role = "SuperUser" Or role = "Operator" Then
                        query = "select p.plateno,p.from_timestamp,p.to_timestamp,p.lat,p.lon,p.userid,u.username,v.groupname,v.pmid from pto_history p Join userTBL u on u.userid=p.userid Join vehicleTBL v On v.plateno=p.plateno and p.userid in (" & userslist & ")  and p.from_timestamp between '" & begindatetime & "' and '" & enddatetime & "' and v.pto='1'"
            ...
            Try
                ...
                While dr.Read
                    ...
                    r(4) = dr("groupname").ToString.ToUpper
                    r(5) = location
                    r(6) = locObj.GetNearestTown(Double.Parse(dr("lat")), Double.Parse(dr("lon")))
                    r(7) = locObj.GetMilePoint(Double.Parse(dr("lat")), Double.Parse(dr("lon")))
                    r(8) = Double.Parse(dr("lat")).ToString("0.0000")
                    r(9) = Double.Parse(dr("lon")).ToString("0.0000")
                    Dim intime As DateTime = Convert.ToDateTime(dr("from_timestamp")).ToString("yyyy/MM/dd HH:mm:ss")
                    r(10) = Convert.ToDateTime(dr("from_timestamp")).ToString("yyyy/MM/dd HH:mm:ss")
                    ...
                    Else
                        ...
                        r(11) = Convert.ToDateTime(dr("to_timestamp")).ToString("yyyy/MM/dd HH:mm:ss")
                        r(12) = "<span style=""color:blue;cursor:pointer;"" onclick=""javascript:DisplayMap('" & intime.ToString("yyyy/MM/dd HH:mm:ss") & "','" & outtime.AddSeconds(-1).ToString("yyyy/MM/dd HH:mm:ss") & "','" & dr("plateno") & "')"">" & (outtime - intime).TotalMinutes.ToString("0") & "</span>"
                        If isAdmin Then
                            t.Rows.Add(r)
                            i = i + 1
                        Else
                            'If (outtime - intime).TotalMinutes > 10 Then
                            t.Rows.Add(r)
                                i = i + 1
                            ' End If
                        End If
            ...
            If (t.Rows.Count > 0) Then
                ...
                sb1.Append("<thead><tr align=""left""><th>S No</th><th>Truck Code</th><th>Plate No</th><th>Username</th><th>Group Name</th><th>Geofence Name/Location</th><th style='Width:140px;'>Nearest Town</th><th>Mile Point</th><th>Lat</th><th>Lon</th><th>From Date Time</th><th>To Date Time</th><th style='Width:60px;'>Duration (Mins)</th></tr></thead>")
                ...
                Next
                sb1.Append("</tbody>")
                sb1.Append("<tfoot><tr align=""left""><th>S No</th><th>Truck Code</th><th>Plate No</th><th>Username</th><th>Group Name</th><th>Geofence Name/Location</th><th>Nearest Town</th><th>Mile Point</th><th>Lat</th><th>Lon</th><th>From Date Time</th><th>To Date Time</th><th>Duration (Mins)</th></tr></tfoot>")
