...
Partial Class PrivateGeofenceSummaryReportTQuarry
        ...
        Try
            ...
            Try
                ...
                While dr.Read
                    ...
                    If CheckForExcavator(dr("plateno"), intime, outtime) Then
                        r(5) = "<span style=""color:green;cursor:pointer;font-weight:bold"" onclick=""javascript:DisplayMap('" & intime.ToString("yyyy/MM/dd HH:mm:ss") & "','" & outtime.ToString("yyyy/MM/dd HH:mm:ss") & "','" & dr("plateno") & "')"">" & (outtime - intime).TotalMinutes.ToString("0") & "</span>"
                    Else
                        r(5) = "<span style=""color:blue;cursor:pointer;"" onclick=""javascript:DisplayMap('" & intime.ToString("yyyy/MM/dd HH:mm:ss") & "','" & outtime.ToString("yyyy/MM/dd HH:mm:ss") & "','" & dr("plateno") & "')"">" & (outtime - intime).TotalMinutes.ToString("0") & "</span>"
                    End If
                    r(6) = dr("username").ToString.ToUpper
                    r(7) = dr("groupname").ToString.ToUpper
                    r(8) = "<span style=""color:blue;cursor:pointer;text-decoration:underline;"" onclick=""javascript:openLog('GetLogDataQuarry.aspx?p=" & dr("plateno") & "&b=" & intime.ToString("yyyy/MM/dd HH:mm:ss") & "&e=" & outtime.ToString("yyyy/MM/dd HH:mm:ss") & "')"">View Log</span>"
                    t.Rows.Add(r)
                    i = i + 1
                End While
    ...
    Private Function CheckForExcavator(ByVal plateno As String, ByVal starttime As String, ByVal endtime As String) As Boolean
        Dim res As Boolean = False
        Dim conn As New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("sqlserverconnection"))
        Try
            Dim exvatorpoints As New List(Of Point)
            Dim vehiclepoints As New List(Of Point)
            Dim exvatordict As New Dictionary(Of Point, DateTime)
            Dim vehicledict As New Dictionary(Of Point, DateTime)
            Dim vehiclepoint As Point
            Dim map As New AspMap.Map()
            Dim cmd As New SqlCommand("select lat,lon,plateno,timestamp from vehicle_history2 where plateno in (select plateno  from vehicletbl where type ='Excavator') and timestamp between @bdt and @edt  order by timestamp", conn)
            cmd.Parameters.AddWithValue("@bdt", Convert.ToDateTime(starttime).ToString("yyyy/MM/dd HH:mm:ss"))
            cmd.Parameters.AddWithValue("@edt", Convert.ToDateTime(endtime).ToString("yyyy/MM/dd HH:mm:ss"))
            conn.Open()
            Dim dr As SqlDataReader = cmd.ExecuteReader()
            While (dr.Read())
                vehiclepoint = New Point
                vehiclepoint.X = dr("lon")
                vehiclepoint.Y = dr("lat")
                exvatorpoints.Add(vehiclepoint)
                exvatordict.Add(vehiclepoint, dr("timestamp"))
            End While
            dr.Close()
            conn.Close()
            cmd.CommandText = "select lat,lon,timestamp from vehicle_history2 where plateno=@plateno and timestamp between @bdt and @edt order by timestamp"
            cmd.Parameters.AddWithValue("@plateno", plateno)

            conn.Open()
            dr = cmd.ExecuteReader()
            Dim countinuty As Boolean = False
            While dr.Read()
                vehiclepoint = New Point
                vehiclepoint.X = dr("lon")
                vehiclepoint.Y = dr("lat")
                vehiclepoints.Add(vehiclepoint)
                vehicledict.Add(vehiclepoint, dr("timestamp"))
            End While
            dr.Close()
            conn.Close()
            Dim starttimefind, endtimefind As DateTime
            Dim found As Boolean = False
            Dim neartrip As Boolean = False
            For Each epoint As Point In exvatorpoints
                found = False
                countinuty = False
                For Each vpoint As Point In vehiclepoints
                    'Response.Write(map.ConvertDistance(map.MeasureDistance(epoint, vpoint), 9102, 9036))
                    'Response.Write(" - ")
                    If map.ConvertDistance(map.MeasureDistance(epoint, vpoint), 9102, 9036) <= 0.02 Then
                        'If countinuty = False Then
                        '    starttimefind = vehicledict(vpoint)
                        'found = True
                        ''Else
                        ''    endtimefind = vehicledict(vpoint)
                        ''End If
                        'countinuty = True
                        res = True
                        Exit For
                ...
                Next
                'If found = True Then
                '    'Response.Write(endtimefind)
                '    'Response.Write(" - ")
                '    'Response.Write(starttimefind)
                '    'Response.Write(" - ")
                '    'Response.Write((endtimefind - starttimefind).TotalMinutes)
                '    'If (endtimefind - starttimefind).TotalMinutes >= 1 Then
                '    res = True
                '    Exit For
                '    ''End If
                'End If
