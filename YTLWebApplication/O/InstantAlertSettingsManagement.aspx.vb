...
Partial Class InstantAlertSettingsManagement
    ...
    Private Sub FillGrid()
        Try
            ...
            Dim key() As String = {"Geofence In", "Geofence Out", "Idling", "Immobilizer", "Overspeed", "Panic", "Power Cut", "PTO On", "Attach", "Detach", "Towing"}
            Dim value() As String = {"geofencein", "geofenceout", "idling", "immobilizer", "overspeed", "panic", "powercut", "alarm", "attach", "detach", "towing"}

            Dim conn As SqlConnection = New SqlConnection(System.Configuration.ConfigurationManager.AppSettings("sqlserverconnection"))
            If Not userid = "--Select User Name--" Then
                Dim cmd As SqlCommand = New SqlCommand("select st.plateno,mobileno,emailid,attach,detach,towing,alarm,geofencein,geofenceout,idling,ignitionon,ignitionoff,st.immobilizer,overspeed,overtimedrive,panic,powercut,unlock,signallose from instant_alert_settings st join vehicleTBL v on v.plateno=st.plateno where v.userid='" & userid & "' order by st.plateno", conn)
                If userid = "--All Users--" Then
                    Dim role As String = Request.Cookies("userinfo")("role")
                    Dim userslist As String = Request.Cookies("userinfo")("userslist")
                    cmd = New SqlCommand("select st.plateno,mobileno,emailid,attach,detach,towing,alarm,geofencein,geofenceout,idling,ignitionon,ignitionoff,st.immobilizer,overspeed,overtimedrive,panic,powercut,unlock,signallose from instant_alert_settings st join vehicleTBL v on v.plateno=st.plateno order by st.plateno", conn)
                    If role = "User" Then
                        cmd = New SqlCommand("select st.plateno,mobileno,emailid,attach,detach,towing,alarm,geofencein,geofenceout,idling,ignitionon,ignitionoff,st.immobilizer,overspeed,overtimedrive,panic,powercut,unlock,signallose from instant_alert_settings st join vehicleTBL v on v.plateno=st.plateno where v.userid='" & userid & "' order by st.plateno", conn)
                    ElseIf role = "SuperUser" Or role = "Operator" Then
                        cmd = New SqlCommand("select st.plateno,mobileno,emailid,attach,detach,towing,alarm,geofencein,geofenceout,idling,ignitionon,ignitionoff,st.immobilizer,overspeed,overtimedrive,panic,powercut,unlock,signallose from instant_alert_settings st join vehicleTBL v on v.plateno=st.plateno where v.userid in(" & userslist & ") order by st.plateno", conn)
                    End If
                End If
                ...
                While dr.Read()
                    ...
                    r(1) = i.ToString()
                    r(2) = "<a href=""UpdateInstantAlertSettings.aspx?plateno=" & dr("plateno") & """/>" & dr("plateno") & "</a>"
                    ...
                    Dim attachactive As String = ""
                    Dim attachinactive As String = ""

                    Dim detachactive As String = ""
                    Dim detachinactive As String = ""

                    Dim towingactive As String = ""
                    Dim towinginactive As String = ""
