    ...
    Protected Sub DisplayMonthInformation()
        Try
            ...
            If username = "ALL" Then
                cmd.CommandText = "select t1.plateno, DAY(dateadd(day,-1,timestamp)) as day,sum(km) as kmtravel,sum(trips) as tripscount,sum(inactivedays) as days from oss_result t1 inner join vehicleTBL t2 on t1.plateno=t2.plateno where  t1.timestamp between '" & ddlyear.SelectedValue & "/" & ddlmonth.SelectedValue & "/01' and '" & ddlyear.SelectedValue & "/" & ddlmonth.SelectedValue & "/" & DateTime.DaysInMonth(ddlyear.SelectedValue, ddlmonth.SelectedValue) & "' group by t1.plateno , DAY(dateadd(day,-1,timestamp))"
            Else
                cmd.CommandText = "select t1.plateno, DAY(dateadd(day,-1,timestamp)) as day,sum(km) as kmtravel,sum(trips) as tripscount,sum(inactivedays) as days from oss_result t1 inner join vehicleTBL t2 on t1.plateno=t2.plateno where t2.userid ='" & username & "' and  t1.timestamp between '" & ddlyear.SelectedValue & "/" & ddlmonth.SelectedValue & "/01' and '" & ddlyear.SelectedValue & "/" & ddlmonth.SelectedValue & "/" & DateTime.DaysInMonth(ddlyear.SelectedValue, ddlmonth.SelectedValue) & "' group by t1.plateno , DAY(dateadd(day,-1,timestamp))"
            End If
            ...
            '                If Not t.DefaultView.Item(i)(j) = "-" Then
            '                    total += t.DefaultView.Item(i)(j)
            '                End If
            '            Next
            '        Catch ex As Exception
            ...
            If (t.Rows.Count > 0) Then
                ...
                For i As Integer = 0 To DateTime.DaysInMonth(ddlyear.SelectedValue, ddlmonth.SelectedValue) - 1
                    ...
                    headcolumns += "<th  class=""ui-state-default"" colspan=3>" & i + 1 & "</th>"
                Next
                sb1.Append("<thead><tr><th class=""ui-state-default"">" & ddlmonth.SelectedValue & " - " & ddlyear.SelectedValue & "</th>" & headcolumns & "<th  class=""ui-state-default"" colspan=3>Total</th></tr></thead>")
                sb1.Append("<thead><tr><th class=""ui-state-default"">Plateno</th>" & columns & "<th class=""ui-state-default"">Trips</th><th class=""ui-state-default"">KM</th><th class=""ui-state-default"">Inactive</th></tr></thead>")
                sb1.Append("<tbody>")
                For i As Integer = 0 To t.Rows.Count - 1
                    Try
                        If i Mod 2 = 0 Then
                ...
                sb1.Append("</tbody>")
                sb1.Append("<tfoot><tr><th class=""ui-state-default"">Plateno</th>" & columns & "<th class=""ui-state-default"">Trips</th><th class=""ui-state-default"">KM</th><th class=""ui-state-default"">Inactive Days</th></tr></tfoot></table>")
            Else
                noDataText = ""
            End If
